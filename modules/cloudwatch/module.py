...
class CloudWatchModule(APIModule):
    ...
    def __init__(self):
        ...
        self.region = cloudwatch_config.get('region', 'us-west-2')
        self.parallel_mode = cloudwatch_config.get('parallel_mode', 'threading')
        self.max_workers = cloudwatch_config.get('max_workers', None)  # None이면 위젯 개수만큼 사용
    ...
    def api(self):
        ...
        parallel_mode에 따라 threading 또는 async 방식으로 처리.
    ...
    def _api_threading(self):
        """CloudWatch 대시보드 데이터를 멀티스레딩으로 조회.
        ...
        # 각 위젯의 메트릭 데이터 조회 (멀티스레딩 병렬 처리)
        widgets = dashboard_def.get('widgets', [])

        # max_workers 결정 (None이면 위젯 개수만큼)
        max_workers = self.max_workers if self.max_workers else len(widgets)

        # 모든 위젯을 병렬로 처리
        widgets_result = []
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # 각 위젯에 대해 process_widget 제출
            future_to_widget = {executor.submit(self.process_widget, widget): widget for widget in widgets}
    ...
    async def _get_dashboard_definition_async(self, dashboard_name):
        """AWS CloudWatch에서 대시보드 정의를 비동기로 가져옵니다.
    ...
    async def _process_widget_async(self, widget):
        """위젯 정의를 처리하고 메트릭 데이터를 비동기로 조회합니다.
        ...
        if widget_type == 'metric':
            metrics = properties.get('metrics', [])
            cloudwatch_config = MODULE_LAYOUT.get(self.name, {})
            default_period = cloudwatch_config.get('default_period', 900)
            period = properties.get('period', default_period)
            stat = properties.get('stat', 'Average')
            ...
            # 메트릭 데이터 비동기 조회
            metric_data = await self._get_metric_data_async(metrics, period, stat)
            widget_info['data'] = metric_data
        else:
            widget_info['data'] = []
    ...
    async def _get_metric_data_async(self, metrics, period, default_stat='Average'):
        """메트릭 데이터를 CloudWatch에서 비동기로 조회합니다 (수학 표현식 지원).
